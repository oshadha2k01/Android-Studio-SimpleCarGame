...
class GameView(var c: Context, var CarGameTask: CarGameTask) : View(c) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val d = resources.getDrawable(R.drawable.c1, null)
        ...
        for (i in otherCars.indices) {
            try {
                ...
                var carY = time - (otherCars[i]["startTime"] as Int) * speed
                val d2 = resources.getDrawable(R.drawable.c2, null)
                ...
                if (otherCars[i]["lane"] as Int == myCarPosition) {
                    if (carY > viewHeight - 2 - carHeight && carY < viewHeight - 2) {
                        CarGameTask.closeGame(score)
                    }
                }
                if (carY > viewHeight) {
                    otherCars.removeAt(i)
                    score++
                    speed = 1 + Math.abs(score / 1000)
                    if (score > highScore) {
                        highScore = score
                    }
                }
